...
Double32_t TrackPar::fgMostProbablePt=kMostProbablePt;
Bool_t TrackPar::fgUseLogTermMS = kFALSE;; 
...
//_____________________________________________________________________________
TrackPar::TrackPar(const TrackPar &track):
  TObject(track),
  fX(track.fX),
  fAlpha(track.fAlpha)
{
...
//_____________________________________________________________________________
TrackPar& TrackPar::operator=(const TrackPar &trkPar)
{
  //
  // assignment operator
  //
...
//_____________________________________________________________________________
TrackPar::TrackPar(Double_t x, Double_t alpha, 
...
//_____________________________________________________________________________
TrackPar::TrackPar(Double_t xyz[3],Double_t pxpypz[3],
...
/*
//_____________________________________________________________________________
void TrackPar::Set(Double_t xyz[3],Double_t pxpypz[3],
				Double_t cv[21],Short_t sign) 
{
  //
...
//_____________________________________________________________________________
void TrackPar::AddCovariance(const Double_t c[15]) {
  //
...
Double_t TrackPar::GetD(Double_t x,Double_t y,Double_t b) const {
  //------------------------------------------------------------------
...
//_______________________________________________________________________
Double_t TrackPar::GetLinearD(Double_t xv,Double_t yv) const {
  //------------------------------------------------------------------
  // This function calculates the transverse impact parameter
...
Bool_t TrackPar::CorrectForMeanMaterialdEdx
...
Bool_t TrackPar::CorrectForMeanMaterial
(Double_t xOverX0,  Double_t xTimesRho, Double_t mass, 
 Bool_t anglecorr,
...
Bool_t TrackPar::CorrectForMeanMaterialZA
(Double_t xOverX0, Double_t xTimesRho, Double_t mass,
 Double_t zOverA,
 Double_t density,
 Double_t exEnergy,
...
Bool_t TrackPar::CorrectForMaterial
...
Double_t TrackPar::BetheBlochAleph(Double_t bg,
         Double_t kp1,
         Double_t kp2,
         Double_t kp3,
         Double_t kp4,
...
Double_t TrackPar::BetheBlochGeant(Double_t bg,
         Double_t kp0,
         Double_t kp1,
         Double_t kp2,
         Double_t kp3,
...
Double_t TrackPar::BetheBlochSolid(Double_t bg) {
  //------------------------------------------------------------------
...
Double_t TrackPar::BetheBlochGas(Double_t bg) {
  //------------------------------------------------------------------
  // This is an approximation of the Bethe-Bloch formula, 
...
Bool_t TrackPar::Rotate(Double_t alpha) {
  //------------------------------------------------------------------
...
Bool_t TrackPar::RotateParamOnly(Double_t alpha)
{
...
Bool_t TrackPar::PropagateTo(Double_t xk, Double_t b) {
  //----------------------------------------------------------------
...
Bool_t TrackPar::PropagateParamOnlyTo(Double_t xk, Double_t b) {
  //----------------------------------------------------------------
  // Propagate this track to the plane X=xk (cm) in the field "b" (kG)
...
Bool_t 
TrackPar::Propagate(Double_t alpha, Double_t x, Double_t b) {
  //------------------------------------------------------------------
  // Transform this track to the local coord. system rotated
  // by angle "alpha" (rad) with respect to the global coord. system, 
...
void TrackPar::Propagate(Double_t len, Double_t x[3],
...
Bool_t TrackPar::Intersect(Double_t pnt[3], Double_t norm[3],
Double_t bz) const {
  //+++++++++++++++++++++++++++++++++++++++++    
  // Origin: K. Shileev (Kirill.Shileev@cern.ch)
...
Double_t 
TrackPar::GetPredictedChi2(const Double_t p[2],const Double_t cov[3]) const {
  //----------------------------------------------------------------
...
Double_t TrackPar::
GetPredictedChi2(const TrackPar *t) const {
  //----------------------------------------------------------------
...
Double_t *TrackPar::GetResiduals(
...
Bool_t TrackPar::Update(const Double_t p[2], const Double_t cov[3]) {
  //------------------------------------------------------------------
...
void 
TrackPar::GetHelixParameters(Double_t hlx[6], Double_t b) const {
  //--------------------------------------------------------------------
...
Double_t TrackPar::GetDCA(const TrackPar *p, 
...
Double_t TrackPar::
PropagateToDCA(TrackPar *p, Double_t b) {
  //--------------------------------------------------------------
...
void TrackPar::GetDirection(Double_t d[3]) const {
  //----------------------------------------------------------------
...
Bool_t TrackPar::GetPxPyPz(Double_t p[3]) const {
  //---------------------------------------------------------------------
...
Double_t TrackPar::PhiPos() const {
  //---------------------------------------------------------------------
...
Double_t TrackPar::Eta() const { 
  // return pseudorapidity

  return -TMath::Log(TMath::Tan(0.5 * Theta())); 
}

Double_t TrackPar::Y() const {
  // return rapidity
...
Bool_t TrackPar::GetXYZ(Double_t *r) const {
  //---------------------------------------------------------------------
...
Bool_t TrackPar::GetCovarianceXYZPxPyPz(Double_t cv[21]) const {
  //---------------------------------------------------------------------
...
Bool_t 
TrackPar::GetPxPyPzAt(Double_t x, Double_t b, Double_t *p) const {
  //---------------------------------------------------------------------
...
Bool_t 
TrackPar::GetYAt(Double_t x, Double_t b, Double_t &y) const {
  //---------------------------------------------------------------------
...
Bool_t 
TrackPar::GetZAt(Double_t x, Double_t b, Double_t &z) const {
  //---------------------------------------------------------------------
...
Bool_t 
TrackPar::GetXYZAt(Double_t x, Double_t b, Double_t *r) const {
  //---------------------------------------------------------------------
...
//_____________________________________________________________________________
void TrackPar::Print(Option_t* /*option*/) const
{
// print the parameters and the covariance matrix

  printf("TrackPar: x = %-12g  alpha = %-12g\n", fX, fAlpha);
...
Double_t TrackPar::GetSnpAt(Double_t x,Double_t b) const {
  //
...
Bool_t TrackPar::GetDistance(TrackPar *param2, Double_t x, Double_t dist[3], Double_t bz){
  //------------------------------------------------------------------------
...
void  TrackPar::DrawTrack(Float_t magf, Float_t minR, Float_t maxR, Float_t stepR){
  //
  // Draw track line
  //
  if (minR>maxR) return ;
...
//
void TrackPar::FillPolymarker(TPolyMarker3D *pol, Float_t magF, Float_t minR, Float_t maxR, Float_t stepR){
  //
...
Int_t TrackPar::GetIndex(Int_t i, Int_t j){
  //
...
void TrackPar::g3helx3(Double_t qfield, 
                                    Double_t step,
                                    Double_t vect[7]) {
/******************************************************************
 *                                                                *
...
Bool_t TrackPar::PropagateToBxByBz(Double_t xk, const Double_t b[3]) {
  //----------------------------------------------------------------
...
Bool_t TrackPar::PropagateParamOnlyBxByBzTo(Double_t xk, const Double_t b[3]) {
  //----------------------------------------------------------------
...
Bool_t TrackPar::Translate(Double_t *vTrasl,Double_t *covV){
  //
...
Bool_t TrackPar::GetXatLabR(Double_t r,Double_t &x, Double_t bz, Int_t dir) const
{
...
Bool_t TrackPar::GetXYZatR(Double_t xr,Double_t bz, Double_t *xyz, Double_t* alpSect) const
{
...
Double_t  TrackPar::GetParameterAtRadius(Double_t r, Double_t bz, Int_t parType) const
{
  //
...
Bool_t TrackPar::Local2GlobalMomentum(Double_t p[3], Double_t alpha) const {
  //----------------------------------------------------------------
...
Bool_t TrackPar::Local2GlobalPosition(Double_t r[3], Double_t alpha) const {
  //----------------------------------------------------------------
  // This function performs local->global transformation of the
  // track position.
  // When called, the arguments are:
...
Bool_t TrackPar::Global2LocalMomentum(Double_t p[3], Short_t charge, Double_t &alpha) const {
  //----------------------------------------------------------------
...
Bool_t TrackPar::Global2LocalPosition(Double_t r[3], Double_t alpha) const {
