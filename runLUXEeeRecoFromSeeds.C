...
void runLUXEeeRecoFromSeeds(TString process, int Seed=12345) //, const char* setup="setup/setupLUXE.txt")
{
   ...
   for(int iev=0;iev<nev and iev<10000;iev++)
   {
      ...
      for(unsigned int iseed=0 ; iseed<pxSeed->size() ; iseed++)
      {
         ...
         det->GetLayer(1)->GetMCCluster()->Set( z1Seed->at(iseed), x1Seed->at(iseed), y1Seed->at(iseed));
         det->GetLayer(3)->GetMCCluster()->Set( z2Seed->at(iseed), x2Seed->at(iseed), y2Seed->at(iseed));
         det->GetLayer(5)->GetMCCluster()->Set( z3Seed->at(iseed), x3Seed->at(iseed), y3Seed->at(iseed));
