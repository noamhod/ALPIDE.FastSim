...
TLegend *trkcolleg()
{
	...
	leg->AddEntry(l11, "1#leq#it{E}[GeV]<2", "l");
	TLine *l12 = new TLine();
	l12->SetLineColor(trkcol(0.5));
	leg->AddEntry(l12, "0.5#leq#it{E}[GeV]<1", "l");
	TLine *l13 = new TLine();
	l13->SetLineColor(trkcol(0.1));
	leg->AddEntry(l13, "#it{E}[GeV]<0.5", "l");
	return leg;
}
...
void WriteGeometry(TPolyMarker3D *&polm, TPolyLine3D *&poll, TString process, int &inacc, TPolyMarker3D *&clusters, TString suff = "")
{
	...
	// /// check acceptance /// check for glitches
	// if(inacc && !skipglitches(polm))
	// {
			// cnv_pl3d->cd();
			// poll->Draw();
			// clusters->Draw();
...
vector<int> clusters_layerid;
vector<int> clusters_type;
TPolyMarker3D *clusters_xyz;
vector<TVector3> clusters_r;
TPolyMarker3D *trkpts_fullrange;
TPolyMarker3D *trkpts;
TPolyLine3D *trklin;
int acc;
TFile *fOut = 0;
TTree *tOut = 0;
...
void KillOutTree()
{
	fOut->cd();
	tOut->Write();
	for (TMapTSTH1D::iterator it = histos1.begin(); it != histos1.end(); ++it)
		it->second->Write();
...
void AddCluster(int slvidx, int index_offset, TString process, TString LYR, double dxMax = 5)
{
	...
	/// !!! dirty fix !!! /// //TODO
	/////////////////////////
	unsigned int ncls = clusters_r.size();
	double xprev = (ncls>0) ? clusters_r[ncls-1].X() : x;
	if(abs(x-xprev)>dxMax)
		return;
	if(x * xprev<0)
		return;
	clusters_xyz->SetNextPoint(x, y, z);
	clusters_r.push_back(v);
	clusters_id.push_back(layerid * index_offset + slvidx); // assuming no chance to have >index_offset tracks (and hence clusters) per tracker arm (and hence per layer)
	clusters_layerid.push_back(layerid);
	clusters_type.push_back((process.Contains("bkg")) ? 0 : 1);
...
int main(int argc, char *argv[])
{
	...
	if(argc==3 and !((TString)argv[2]).Contains("-sigl=")) { printf("argc=3 but cannot parse %s\n", argv[2]); exit(-1); }
	...
	for(int s = 0; s<sides.size(); s++)
	{
		...
		hname = "h2_E_vs_x_L1O_" + side;
		histos2.insert(make_pair(hname, new TH2D(hname, ";x_{1} [cm];E [GeV];Tracks", 500, -55, +55, 340, 0, +17)));
		hname = "h2_E_vs_x_L4I_" + side;
		histos2.insert(make_pair(hname, new TH2D(hname, ";x_{4} [cm];E [GeV];Tracks", 500, -55, +55, 340, 0, +17)));
		hname = "h2_E_vs_x_L4O_" + side;
		histos2.insert(make_pair(hname, new TH2D(hname, ";x_{4} [cm];E [GeV];Tracks", 500, -55, +55, 340, 0, +17)));

		hname = "h2_dx14_vs_x_L4I_" + side;
		histos2.insert(make_pair(hname, new TH2D(hname, ";x_{4} [cm];|x_{4}-x_{1}| [cm];Tracks", 600, -55, +55, 400, 0, +8)));
		hname = "h2_dx14_vs_x_L4O_" + side;
		histos2.insert(make_pair(hname, new TH2D(hname, ";x_{4} [cm];|x_{4}-x_{1}| [cm];Tracks", 600, -55, +55, 400, 0, +8)));
		...
		for (int iev = (fullloop) ? 0 : evnt; (fullloop) ? iev<10 : iev==evnt; iev++)
		{
			...
			for(int igen = 0; igen<ngenall; igen++)
			{
				...
				crg  = q;
				trkp4 = ptmp;
				if(!process.Contains("bkg")) trkpts_fullrange = TrackMarker3d(trutrk, 0, zLastLayer + 15, 0.1, trkcol(ptmp.E()), false, true);
				trkpts = TrackMarker3d(trutrk, 0, zLastLayer + 15, 0.1, trkcol(ptmp.E()));
				trklin = TrackLine3d(trutrk, zLastLayer + 15, 1, trkcol(ptmp.E()));
				...
				for (Int_t n = 0; n<trkpts_fullrange->GetN(); ++n)
				{
					Double_t xp, yp, zp;
					trkpts_fullrange->GetPoint(n, xp, yp, zp);
					...
					if(zp==zfirstinner && isInnerX) histos2["h2_E_vs_x_L1I_" + side]->Fill(xp, trkp4.E());
					if(zp==zlastinner && isInnerX)  histos2["h2_E_vs_x_L4I_" + side]->Fill(xp, trkp4.E());
					if(zp==zfirstouter && isOuterX) histos2["h2_E_vs_x_L1O_" + side]->Fill(xp, trkp4.E());
					if(zp==zlastouter && isOuterX)  histos2["h2_E_vs_x_L4O_" + side]->Fill(xp, trkp4.E());
				...
				if(x4I>xMinI && x4I<xMaxI)
				{
					if((x1I>xMinI && x1I<xMaxI))
					{
						...
						histos2["h2_dy14_vs_y_L4I_" + side]->Fill(y4I, y4I-y1I);
					}
				}
				...
				if(x4O>xMinO && x4O<xMaxO)
				{
					if((x1O>xMinO && x1O<xMaxO))
					{
						...
						histos2["h2_dy14_vs_y_L4O_" + side]->Fill(y4O, y4O-y1O);
					}
					else if((x1I>xMinI && x1I<xMaxI))
					{
						histos2["h2_dx14_vs_x_L4X_" + side]->Fill(x4O, abs(x4O-x1I));
						histos2["h2_dy14_vs_y_L4X_" + side]->Fill(y4O, y4O-y1I);
					}
				}
				...
				if(x1I>xMinI && x1I<xMaxI)
				{
					if((x4I>xMinI && x4I<xMaxI))
					{
						...
						histos2["h2_dy14_vs_y_L1I_" + side]->Fill(y1I, y4I-y1I);
					}
					else if((x4O>xMinO && x4O<xMaxO))
					{
						histos2["h2_dx14_vs_x_L1X_" + side]->Fill(x1O, abs(x4O-x1I));
						histos2["h2_dy14_vs_y_L1X_" + side]->Fill(y1O, y4O-y1I);
					}
				}
				...
				if(x1O>xMinO && x1O<xMaxO)
				{
					if((x4O>xMinO && x4O<xMaxO))
					{
						...
						histos2["h2_dy14_vs_y_L1O_" + side]->Fill(y1O, y4O-y1O);
					}
				}
				...
				for (unsigned int k = 0; k<layersnames.size(); k++)
				{
					...
					if(crg<0 && LYR.Contains("P")) continue;
					if(crg>0 && LYR.Contains("E")) continue;
					AddCluster(slvidx, index_offset, process, LYR);
				}

				int nclusters = clusters_layerid.size(); // same as layers hit by the track
